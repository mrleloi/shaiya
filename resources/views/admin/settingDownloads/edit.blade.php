    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.setting-downloads.update", [$settingDownload->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required" for="header">{{ trans('cruds.settingDownload.fields.header') }}</label>
                    <input class="form-control {{ $errors->has('header') ? 'is-invalid' : '' }}" type="text" name="header" id="header" value="{{ old('header', $settingDownload->header) }}" required>
                ...
                <div class="form-group">
                    <label for="link_gdrive">{{ trans('cruds.settingDownload.fields.link_gdrive') }}</label>
                    <textarea class="form-control {{ $errors->has('link_gdrive') ? 'is-invalid' : '' }}" name="link_gdrive" id="link_gdrive">{{ old('link_gdrive', $settingDownload->link_gdrive) }}</textarea>
                    @if($errors->has('link_gdrive'))
                        <span class="text-danger">{{ $errors->first('link_gdrive') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.settingDownload.fields.link_gdrive_helper') }}</span>
                </div>
                <div class="form-group">
                    <label for="link_mega">{{ trans('cruds.settingDownload.fields.link_mega') }}</label>
                    <textarea class="form-control {{ $errors->has('link_mega') ? 'is-invalid' : '' }}" name="link_mega" id="link_mega">{{ old('link_mega', $settingDownload->link_mega) }}</textarea>
                    @if($errors->has('link_mega'))
                        <span class="text-danger">{{ $errors->first('link_mega') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.settingDownload.fields.link_mega_helper') }}</span>
                </div>
                <div class="form-group">
